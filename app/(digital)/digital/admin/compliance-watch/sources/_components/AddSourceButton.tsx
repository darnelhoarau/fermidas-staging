...
export function AddSourceButton({ productId }: { productId: string }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Clear validation results when opening
      setUrlValidationResults(new Map());
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/admin/compliance-watch/sources', {
        method: 'POST',
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4'>
      <div className='card max-h-[90vh] w-full max-w-2xl overflow-y-auto p-8'>
        <div className='mb-6 flex items-center justify-between'>
          <h2 className='text-2xl font-bold text-brand'>Add New Source</h2>
          <button
        ...
        <form onSubmit={handleSubmit} className='space-y-6'>
          ...
          <div>
            ...
            <input
              ...
              name='name'
              required
              placeholder='e.g. Seychelles Official Gazette'
              className='w-full rounded-lg border border-leaf-300 px-4 py-2 text-brand focus:border-leaf-600 focus:outline-none focus:ring-2 focus:ring-leaf-600/20'
            />
          </div>
